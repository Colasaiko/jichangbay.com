...
# 3. Update tr and card data attributes
def replacer_tr(match):
    return 'providers.forEach(function(p, index) { %>' + match.group(1) + """
                  let numPrice = 999999;
                  if (firstPlan) {
...
html = re.sub(r'providers\.forEach\(function\(p\) \{ %>(.*?%>)\s*<tr>', replacer_tr, html, flags=re.DOTALL)

# Mobile cards also need the same class logic
...
html = re.sub(r'providers\.forEach\(function\(p\) \{ %>(.*?%>)\s*<div class="mobile-card">', replacer_card, html, flags=re.DOTALL)
